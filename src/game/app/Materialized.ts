 ...
 */
export function Materialized<T extends Constructor>(Base: T): T {
  return class extends Base implements Materializable {
    ...
    protected onCollision(_other: Body): void { return }

    /**
     * called once before `fixedUpdate` ticks start
     */
    protected onLoad(_physicsBody: Body): void { return }

    /**
     * use a semi fixed timestep ticker with a maximum frequency of 60Hz (60fps);
     * recommended for game logics & physics
     */
    protected fixedUpdate(_delta: number): void { return }
  }
}